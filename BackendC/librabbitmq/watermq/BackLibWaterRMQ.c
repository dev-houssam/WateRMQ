...
// Dans la prochaine version de ce programme
// Il faut : utiliser un fork et detacher le processus
// Mettre en place un header : 
// Le header va indiquer à C++ quel est le PID 
// du programme qui lui fourni les données (payload)
// Le programme C++ devra aussi vérrouiller le program C.


//Une version du programme devra fournir
// un UI web pour réaliser certaines configurations :
...
int main(int argc, const char * argv[])
{
  ...
  //atoi = ascii to int : pour la conversion si necessaire
  //utilisation : port, channel_max, frame_max, heartbeat etc..
  ...
  //Les configurations semaphore sont importants
  //pour sécuriser la section critique : le tunnel
  //pour synchroniser le backend et le frontend
  configurationSemaphoreMmap_private(wrmq_info); 
